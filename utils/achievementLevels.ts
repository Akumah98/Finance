// Frontend copy of achievement level thresholds and helpers
...
export const getTierEmoji = (level: number): string => {
    if (level <= 20) return '🥉';
    if (level <= 40) return '🥈';
    if (level <= 60) return '🥇';
    if (level <= 80) return '💎';
...
export const calculateLevel = (achievementType: keyof typeof ACHIEVEMENT_LEVELS, count: number): number => {
...
export const getProgressToNextLevel = (achievementType: keyof typeof ACHIEVEMENT_LEVELS, count: number) => {
    const level = calculateLevel(achievementType, count);
    if (level >= 100) return { current: count, target: count, progress: 100, level: 100 };

    const thresholds = ACHIEVEMENT_LEVELS[achievementType];
    const currentThreshold = level === 1 ? 0 : thresholds[level - 2];
